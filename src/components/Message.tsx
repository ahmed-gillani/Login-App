...
const Message: React.FC<MessageProps> = ({
    ...
    const [typedBulletIndex, setTypedBulletIndex] = useState<number>(0);
    const [isTyping, setIsTyping] = useState<boolean>(false);

    //   const { data: messageDetails } = useMessageDetails<MessageDetails>(messageId);
    ...
    return (
        <div className="group py-6 px-4 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition">
            <div
                className={`flex gap-5 items-start max-w-4xl mx-auto ${isUser ? "justify-end" : "justify-start"
                    }`}
            >
                {!isUser && (
                ...
                <div className={isUser ? "max-w-lg" : "flex-1 w-full max-w-3xl"}>
                    <div
                        className={`relative px-6 py-4 rounded-3xl shadow-lg inline-block ${isUser
                    ...
                    >
                        <div
                            className={`absolute top-0 w-4 h-4 transform ${isUser
