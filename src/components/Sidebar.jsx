...
import {
  Menu, X, Plus, Search, MessageSquare, Trash2, Sun, Moon
} from "lucide-react";

export default function Sidebar({ conversations, activeId, onSelect, onNewChat, onDelete }) {
  const [openMobile, setOpenMobile] = useState(false);
  const [theme, setTheme] = useState(localStorage.getItem("theme") || "light"); // Default light to match picture
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`fixed md:static top-0 left-0 h-full w-72 bg-gray-50 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 shadow-xl z-50 flex flex-col transition-transform duration-300 ${openMobile ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        ...
        <div className="p-5 border-b border-gray-200 dark:border-gray-800">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl font-bold text-gray-900 dark:text-white">Assistant</h1>
            </div>
            <button onClick={() => setOpenMobile(false)} className="md:hidden p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-800">
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="p-4">
          <button
            onClick={() => { onNewChat(); setOpenMobile(false); }}
        ...
        <div className="flex-1 overflow-y-auto px-4 space-y-1">
          {filteredConversations.length === 0 ? (
            ...
            filteredConversations.map((conv) => (
              <div
                key={conv.id}
                onClick={() => { onSelect(conv.id); setOpenMobile(false); }}
        ...
        {/* Theme Toggle at Bottom - Exactly like your picture */}
